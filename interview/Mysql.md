...
+ Mysql的几种索引
  ...
  + 应用层次来区分有普通索引，唯一索引，复合索引
    ...
    + 聚簇索引：聚簇索引就是叶子节点就是数据节点，叶子节点存储了数据。聚集索引在一个表里面只允许有一个，同时存储的索引记录在物理层面上也是连续的，也就是在我们查找到某一列的数据的时候，我们可以按照索引的连续性很快查找到其他的数据。
    + 非聚簇索引：叶子节点还是索引节点，不过存储了对应的数据上的指针。非聚集索引不要求连续，他只是在逻辑地址上面的连续，不要求物理地址的连续性。
